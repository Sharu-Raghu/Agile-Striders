...
/**
 * Created by Sharanya on 12/04/2018.
 */
...
public class NoteActivity extends AppCompatActivity {
    ...
    private void actionCancel() {
        ...
        if(!checkNoteAltred()) { //if note is not altered by user (user only viewed the note/or did not write anything)
            finish(); //just exit the activity and go back to TipsActivity
            ...
            AlertDialog.Builder dialogCancel = new AlertDialog.Builder(this)
                    ...
                    .setMessage("Are you sure you do not want to save the changes?")
                    .setPositiveButton("YES", new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
    ...
    /**
     * Validate the title and content and save the note and finally exit the activity and go back to TipsActivity
     */
    private void validateAndSaveNote() {
        ...
        finish(); //exit the activity, should return us to TipsActivity
    }
}
